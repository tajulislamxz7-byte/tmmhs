...
export async function renderAbout() {
  ...
  const settings = S || JSON.parse(localStorage.getItem('gfa_settings') || '{}');
  const allUsers = users || JSON.parse(localStorage.getItem('gfa_users_cache') || '[]');

  const name     = settings.name      || schoolInfo.name;
  const tagline  = settings.tagline   || schoolInfo.tagline;
  const founded  = settings.founded   || schoolInfo.founded;
  const address  = settings.address   || schoolInfo.address;
  ...
  // Facilities — prefer settings, fall back to schoolInfo
  const facilities = settings.facilities && settings.facilities.length > 0
    ? settings.facilities
    : schoolInfo.facilities;

  // About page sections from settings
  const aboutPage = settings.aboutPage || {};
  const history = aboutPage.history || 'A proud institution serving the community since ' + founded + '.';
  const mission = aboutPage.mission || 'To give every student a rigorous, well-rounded education that prepares them for university and for life — regardless of background.';
  const vision = aboutPage.vision || 'To be recognized as one of Bangladesh\'s leading centers of academic excellence and civic character by 2030.';
  const coreValues = aboutPage.coreValues || 'Integrity, Curiosity, Excellence, Community, and Respect — these five values guide every interaction within our school walls.';
  const faqs = aboutPage.faqs || [
    { question: 'What is the admission process for Class 6?', answer: 'Applications open in August each year, followed by a written test and interview. Results are published within two weeks of the test date.' },
    { question: 'Does the school offer scholarships?', answer: 'Yes — merit scholarships are available for Classes 6 through 10. Need-based scholarships are also reviewed case by case by the admissions committee.' },
    { question: 'What are the school hours?', answer: 'Classes run Sunday to Thursday, 8:00 AM to 2:30 PM, with extended hours for Class 9–10 during exam terms.' },
    { question: 'How can I contact my child\'s class teacher?', answer: 'Guardians can message teachers directly through the Messages tab once their student profile is linked to a guardian account.' },
    { question: 'Is there a dress code?', answer: 'Yes. Students wear the official school uniform Monday through Thursday. Friday is free-dress day within school guidelines.' },
    { question: 'How are exam results shared?', answer: 'Results are published on the student portal and as printed marksheets distributed through class teachers. PDF marksheets can be downloaded anytime.' },
  ];
    ...
    <div class="page-container">
      ...
      <div class="container section-sm">
        ...
        <div class="grid-2 gap-6 mb-12">
          ${[
            {svg:'<rect x="3" y="3" width="18" height="18" rx="2"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="9" y1="21" x2="9" y2="9"/>', title:'Our History',  text:history},
            {svg:'<path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/>',                                   title:'Mission',      text:mission},
            {svg:'<circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>',           title:'Vision',       text:vision},
            {svg:'<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/>',                  title:'Core Values',  text:coreValues},
          ].map(item=>`
            <div class="card">
            ...
            ${faqs.map((f,i)=>`
              <div style="border:1.5px solid var(--border);border-radius:var(--radius-xl);overflow:hidden;margin-bottom:10px;background:var(--bg-primary);">
                <button onclick="toggleFAQ(${i})" style="width:100%;display:flex;align-items:center;justify-content:space-between;padding:18px 20px;text-align:left;font-size:var(--text-sm);font-weight:600;color:var(--text-primary);background:none;border:none;cursor:pointer;">
                  <span style="display:flex;align-items:center;gap:10px;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>${f.question}</span>
                ...
                <div id="faq-body-${i}" style="display:none;padding:0 20px 18px;font-size:var(--text-sm);color:var(--text-secondary);line-height:1.7;">${f.answer}</div>
              </div>
            `).join('')}
          </div>
        </div>
